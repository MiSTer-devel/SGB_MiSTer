...
constant SGB_REG_BASE  = $5000
constant SGB_RAM_BASE  = $5800
constant SGB_PKT_BASE  = $7000
...
constant SS_SGBREGS   = $0C
constant SS_SGBRAM    = $0D
...
-
	...
	lda.w SGB_REG_BASE,x	;// Load value from register
	
	sta SSDATA				;// Store register value in save state
	
	cpx #$000F
	beq Save_sgb_packets
	inx
	bra -
...
-
	...
	lda.w SGB_PKT_BASE,y	;// Load value from register
	
	sta SSDATA				;// Store register value in save state
	
	cpy #$000F
	beq Save_sgb_packets_end
	inx
	iny
	bra -
Save_sgb_packets_end:
...
Save_sgb_ram:
	...
	lda.b #(SGB_RAM_BASE >> 8)
	xba
	lda.b #(SGB_RAM_BASE)
	tcd						;// Set Direct page to SGB_RAM_BASE

	ldx #$0000
-
	lda $00,x				;// Read $5800-$5FFF
	sta SSDATA
